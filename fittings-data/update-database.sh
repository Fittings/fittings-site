...
if [ ! -f ./dist/fittings.db ];
  then
    ...
    sqlite3 ./dist/fittings.db < ./init-scripts/000.001.history.sql
    sqlite3 ./dist/fittings.db "INSERT INTO update_history(major_version, minor_version, description, script_type, script_name, run_time)
...
max_major=$(sqlite3 ./dist/fittings.db "select max(major_version) from update_history where script_type='UPDATE'")
max_minor=$(sqlite3 ./dist/fittings.db "select max(minor_version) from update_history where script_type='UPDATE'")
cd update-scripts
...
do
  ...
  then
    if sqlite3 ../dist/fittings.db "INSERT INTO update_history(major_version, minor_version, description, script_type, script_name, run_time)
